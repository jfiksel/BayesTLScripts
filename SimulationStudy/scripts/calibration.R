...
sample.M2Informed <- function(B, gamma.vec, epsilon, T.mat, X) {
    C <- nrow(T.mat)
    M <- matrix(NA, ncol = C, nrow = C)
    for(j in 1:nrow(M)){
        alpha <- B[j,] + gamma.vec[j] * (X[j,] + matrix(rep(epsilon, C), 1, C)) + T.mat[j,]
        M[j,] <- rdirichlet(1, alpha)
    }
    return(M)
}
...
log.pi2Informed <- function(gamma.val, e, alpha, beta, M, M.row, tol=.00001, Xrow) {
    C <- ncol(M)
    gammafn.q1 <- C * gamma.val * e + gamma.val * sum(Xrow == 1) + tol
    gammafn.q2 <- sapply(seq_along(Xrow), function(i) {
        gamma.val * e + gamma.val * (Xrow[i] == 1) + tol
    })
    first.term <-
        log(gamma(gammafn.q1)) - sum(log(gamma(gammafn.q2)))
    second.term <- (alpha - 1) * log(gamma.val + tol) - beta * gamma.val
    M.vec <- M[M.row,]
    third.term <- sum(sapply(seq_along(M.vec), function(i) {
        return((gamma.val * e + gamma.val * (Xrow[i] == 1)) * log(M.vec[i] + tol))
    }))
    return(first.term + second.term + third.term)
}
...
sample.gamma2Informed <- function(gamma.vec, epsilon, alpha, beta, M, tau.vec, max.gamma, X) {
    ### will use log scale for numeric stability
    # gamma.prop <- rgamma(1, shape = gamma.t * tau, rate = tau)
    # g.num <- dgamma(gamma.t, shape = gamma.prop * tau, rate = tau, log = TRUE)
    # g.denom <- dgamma(gamma.prop, shape = gamma.t * tau, rate = tau, log = TRUE)
    sampled.gammas <- sapply(seq_along(gamma.vec), function(i) {
        ...
        g.num <- dlnorm(gamma.t, meanlog = log(gamma.prop), sdlog = tau, log = TRUE)
        g.denom <- dlnorm(gamma.prop, meanlog = log(gamma.t), sdlog = tau, log = TRUE)
        pi.num <- log.pi2Informed(gamma.prop, epsilon, alpha, beta, M, M.row = i, Xrow = X[i,])
        pi.denom <- log.pi2Informed(gamma.t, epsilon, alpha, beta, M, M.row = i, Xrow = X[i,])
        log.alpha <- g.num + pi.num - g.denom - pi.denom
        accept.prob <- min(1, exp(log.alpha))
        u <- runif(1)
        if(u <= accept.prob) {
...
revamp.sampler <- function(test.cod, calib.cod, calib.truth, causes,
  ...
  if(informed) {
      post.samples[[1]]$gamma <- sample.gamma2Informed(rep(gamma.init, nrow(T.mat)), epsilon,
  ...
  for(i in 2:ndraws){
    if(informed) {
        post.samples[[i]]$M <- sample.M2Informed(post.samples[[i-1]]$B,
                                                 post.samples[[i-1]]$gamma,
                                                 epsilon, T.mat, X)
    } else {
    ...
    if(informed) {
        post.samples[[i]]$gamma <- sample.gamma2Informed(post.samples[[i-1]]$gamma,
                                                         epsilon, alpha, beta,
...
revamp.ensemble.lite.sampler <- function(test.cod.mat, calib.cod.mat, calib.truth, causes,
  ...
  for(k in 1:K) {
    ...
    M.mat <- post.samples[[1]]$M.array[,,k]
    if(informed) {
        post.samples[[1]]$gamma.mat[,k] <- sample.gamma2Informed(rep(gamma.init, nrow(T.mat)),
                                                         epsilon,
                                                         alpha, beta, M.mat, tau.vec,
                                                         max.gamma, X) 
    } else {
  ...
  for(i in 2:ndraws){
      if(informed) {
          post.samples[[i]]$M.array <- sapply(1:k, function(k) 
              sample.M2Informed(post.samples[[i-1]]$B.array[,,k], 
                        post.samples[[i-1]]$gamma.mat[,k], epsilon, T.array[,,k], X),
              simplify="array") 
      } else {
          post.samples[[i]]$M.array <- sapply(1:k, function(k) 
              sample.M2(post.samples[[i-1]]$B.array[,,k], 
                        post.samples[[i-1]]$gamma.mat[,k], epsilon, T.array[,,k]),
              simplify="array") 
      }
    ...
    for(k in 1:K) {
      M.mat <- post.samples[[i]]$M.array[,,k]
      if(informed) {
          post.samples[[i]]$gamma.mat[,k] <- sample.gamma2Informed(post.samples[[i-1]]$gamma.mat[,k],
                                                           epsilon, alpha, beta, M.mat, tau.vec, max.gamma, X) 
      } else {
          post.samples[[i]]$gamma.mat[,k] <- sample.gamma2(post.samples[[i-1]]$gamma.mat[,k],
