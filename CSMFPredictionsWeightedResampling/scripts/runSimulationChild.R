...
sim.dir <- here("CSMFPredictionsWeightedResampling", "cluster_output_child")
...
### CSMF  data frame
if(setting$alpha == 5 & setting$cod == 5) {
    ### Tariff with training + calibration
    set.seed(123)
    tariff.train.calib <- codeVA(data = test.final,
                                 data.type = "customize", model = "Tariff",
                                 data.train = rbind(train.final, calib.final),
                                 causes.train = "Cause")
    tariff.train.and.calib.cod <- changeTopCOD(getTopCOD(tariff.train.calib)[,2])
    ...
    insilico.train.calib <- codeVA(data = test.final,
                                   ...
                                   data.train = rbind(train.final, calib.final),
                                   causes.train = "Cause",
                                   jump.scale = 0.05, Nsim=insilico.nsim,
                                   auto.length = FALSE)
    insilico.train.and.calib.cod <- changeTopCOD(getTopCOD(insilico.train.calib)[,2])
    tariff.train.calib.csmf <- openVACSMF(tariff.train.and.calib.cod, causes)
    insilico.train.calib.csmf <- openVACSMF(insilico.train.and.calib.cod, causes)
    ...
    csmf.df <- data.frame(csmf.est = c(tariff.calibva.csmf,
        ...
        select(cause, method, csmf.est, true.csmf)
} else {
    methods <- c("tariff_calibva",
                 "insilico_calibva",
                 "ensemble_calibva",
